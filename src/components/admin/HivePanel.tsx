    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between rounded-xl border border-surface-800 bg-surface-900/50 p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-surface-500">Platform intelligence layer</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {status?.online && (
            <span className="flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-2.5 py-1 text-[10px] font-medium text-emerald-400 border border-emerald-500/20">
      ...
      {status && status.topTopics.length > 0 && (
        <div className="rounded-xl border border-surface-800 bg-surface-900/50 p-4">
          <p className="mb-2 text-[10px] font-medium text-surface-500 uppercase tracking-wider">Learned Topics</p>
      ...
      {status && status.recentLearnings.length > 0 && (
        <div className="rounded-xl border border-surface-800 bg-surface-900/50 p-4">
          <p className="mb-2 text-[10px] font-medium text-surface-500 uppercase tracking-wider">Recent Learnings</p>
              ...
              <li key={i} className="text-xs text-surface-400 leading-relaxed">
                <span
                  className={cn(
                    ...
                    l.source === "internal" ? "bg-emerald-500/10 text-emerald-400" : "bg-cyan-500/10 text-cyan-400"
                  )}
                >
                  {l.source}
                </span>
      ...
      <div className="rounded-xl border border-surface-800 bg-surface-900/50 p-4 space-y-2">
        <button
          onClick={handleSweep}
          disabled={sweeping}
          className="flex w-full items-center justify-center gap-2 rounded-lg bg-amber-500/10 border border-amber-500/20 px-4 py-2 text-sm font-medium text-amber-400 transition-colors hover:bg-amber-500/20 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {sweeping ? <Loader2 className="h-4 w-4 animate-spin" /> : <Zap className="h-4 w-4" />}
          {sweeping ? "Learning from platform..." : "Run Learning Sweep"}
        </button>
        {sweepResult && (
          <p className="text-[10px] text-surface-500 leading-relaxed">{sweepResult}</p>
        )}
