...
export default defineConfig([
  ...
  globalIgnores([
    ...
    "prisma/**",
    "public/**",
    // Convex codegen. Regenerated by `convex dev`/`deploy`, never hand-edited,
    // and not ours to style — linting it only produces noise that buries real
    // findings (it carried 4 files' worth of problems on every local run).
    "convex/_generated/**",
    // The freebuff local tooling workspace: gitignored scratch that includes a
    // vendored third-party pg client. It is not part of the repo, so a local
    // `npm run lint` must not be graded on it — before this ignore, 716 of the
    // 718 reported errors came from here and from generated code, which is
    // exactly how two real errors in `src/` stayed invisible.
    ".freebuff/**",
  ]),
]);